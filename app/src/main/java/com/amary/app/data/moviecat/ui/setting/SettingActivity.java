...
import com.amary.app.data.moviecat.ui.main.MainActivity;
...
public class SettingActivity extends BaseActivity implements CompoundButton.OnCheckedChangeListener {
    ...
    private void getSwitchRm(){
        settingAlarm = new SettingAlarm(this);
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        switch (buttonView.getId()){
            case R.id.sw_dailiy_reminder:
                if (isChecked){
                    settingAlarm.startDailyRm(getString(R.string.app_name), getString(R.string.daily_message));
                    saveDailyRm(true);
                }else {
                    settingAlarm.stopDailyRm();
                    saveDailyRm(false);
                }
                break;
            case R.id.sw_release_reminder:
                if (isChecked){
                    settingAlarm.startReleaseRm();
                    saveReleaseRm(true);
                }else {
                    settingAlarm.stopReleaseRm();
                    saveReleaseRm(false);
                }
                break;
        }
